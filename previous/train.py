...
def normalize_hand(hand_arr: np.ndarray) -> np.ndarray:
    """Normalize hand keypoints: wrist-relative + scale normalize.
    Input: (42,) flat array of (x,y) pairs in pixel coords.
    Output: (42,) normalized, wrist at origin, scale=1.
    """
    if not np.any(hand_arr != 0):
        return hand_arr
    pts = hand_arr.reshape(21, 2).copy()
    wrist = pts[0].copy()
    pts -= wrist                          # wrist → origin
    scale = np.linalg.norm(pts[9] - pts[0])  # middle finger MCP distance
    if scale > 1e-6:
        pts /= scale
    return pts.flatten()
...
def load_pose_data(db_path: str) -> tuple:
    """Load poseDataset with normalized hand features."""
    ...
    for row in rows:
        ...
        # Normalize hands
        raw[:42]  = normalize_hand(raw[:42])   # right hand
        raw[42:84] = normalize_hand(raw[42:84]) # left hand
        # Normalize pose: scale by image width estimate (~1280)
        raw[84:] /= 1280.0
        X_list.append(raw)
