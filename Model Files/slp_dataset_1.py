...
class SLPDataset(Dataset):
    ...
    def __init__(self, split: str = "train"):
        ...
        for vid_id, info in nslt.items():
            ...
            if vid_id in missing:
                continue
            slp_dir  = os.path.join(cfg.DATASET_ROOT, "keypoints_slp")
            kp_dir   = slp_dir if os.path.isdir(slp_dir) else cfg.KEYPOINTS_DIR
            npy_path = os.path.join(kp_dir, f"{vid_id}.npy")
    ...
    def __getitem__(self, idx):
        ...
        seq = np.load(npy_path).astype(np.float32)   # (T, 225) full keypoints - NO slicing for SLP
